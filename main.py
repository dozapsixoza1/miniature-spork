...
async def edit_message(call: types.CallbackQuery, text: str, markup):
    """Helper: edit caption if photo message, else edit text"""
    try:
        await call.message.edit_caption(caption=text, parse_mode="Markdown", reply_markup=markup)
    except Exception:
        try:
...
async def process_bet(call: types.CallbackQuery):
    ...
    if balance < bet:
        await call.answer(f"❌ Недостаточно средств! Баланс: ${balance:.2f}", show_alert=True)
        return

    if game == "coin":
        await edit_message(call,
            f"🪙 *МОНЕТКА*\n\nСтавка: `${bet}`\n\nВыбери сторону монетки:",
            coin_choice_keyboard(bet)
        )
    elif game == "roulette":
        await edit_message(call,
            f"🎡 *РУЛЕТКА*\n\nСтавка: `${bet}`\n\nВыбери тип ставки:",
            roulette_keyboard(bet)
        )
    elif game == "slots":
...
async def all_in(call: types.CallbackQuery):
    ...
    if balance <= 0:
        ...
        return
    call.data = f"bet_{game}_{balance}"
    await process_bet(call)


async def custom_bet_handler(call: types.CallbackQuery, state: FSMContext):
    game = call.data.split("_")[2]
    balance = await db.get_balance(call.from_user.id)
    await state.set_state(States.waiting_bet_amount)
    await state.update_data(game=game)
    await call.message.answer(
        f"✏️ *Введите сумму ставки:*\n\n💰 Ваш баланс: `${balance:.2f}`\nМинимум: $1",
        parse_mode="Markdown",
        reply_markup=back_keyboard("games_menu")
    )
    await call.answer()


@dp.message(States.waiting_bet_amount)
async def process_custom_bet(message: types.Message, state: FSMContext):
    try:
        ...
        # Create a fake callback to reuse process_bet logic
        # We send a message with the game result directly
        class FakeCall:
            data = f"bet_{game}_{bet}"
        ...
        if game == "coin":
            ...
            await run_slots_from_message(message, bet)
        elif game == "dice":
            await run_dice_from_message(message, bet)
        elif game == "mines":
            await run_mines_from_message(message, bet)
        elif game == "crash":
            await run_crash_from_message(message, bet)
        elif game == "blackjack":
            await run_blackjack_from_message(message, bet)
        elif game == "bulls":
            await run_bulls_from_message(message, bet)

    except ValueError:
        await message.answer("❌ Введите корректное число")


async def run_slots_from_message(message: types.Message, bet: float):
    ...
    await db.record_game(user_id, "slots", bet, winnings, win)
...
async def run_dice_from_message(message: types.Message, bet: float):
    user_id = message.from_user.id
    balance = await db.get_balance(user_id)
    if balance < bet:
    ...
    dice_msg = await message.answer("🎲 Бросаем кости...")
...
async def run_mines_from_message(message: types.Message, bet: float):
    user_id = message.from_user.id
    balance = await db.get_balance(user_id)
    if balance < bet:
        await message.answer(f"❌ Недостаточно средств!")
    ...
    text = f"💣 *МИНЫ — Раунд начат!*\n━━━━━━━━━━━━━━━━━━━━\nСтавка: `${bet:.2f}`\nМин на поле: 5\nТекущий множитель: x1.00\n\nОткрывай клетки! 💎 = алмаз, 💣 = мина\n━━━━━━━━━━━━━━━━━━━━"
    keyboard = generate_mines_keyboard(board, user_id, bet, opened=[])
    result_msg = await message.answer(text, parse_mode="Markdown", reply_markup=keyboard)
    await db.update_mines_message(user_id, result_msg.message_id)


async def run_crash_from_message(message: types.Message, bet: float):
    user_id = message.from_user.id
    balance = await db.get_balance(user_id)
    if balance < bet:
        await message.answer(f"❌ Недостаточно средств!")
    ...
    msg = await message.answer(f"🚀 *КРАШ*\n━━━━━━━━━━━━━━━━━━━━\nСтавка: `${bet:.2f}`\n\nРакета взлетает...\n\nМножитель: x1.00", parse_mode="Markdown")
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="💰 Забрать (x1.00)", callback_data=f"crash_cashout_{bet}_1.00_{msg.message_id}"))
    ...
    while multiplier < crash_point:
        ...
        if multiplier > 5: step = 0.2
        if multiplier > 10: step = 0.5
        current_win = bet * multiplier
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text=f"💰 Забрать (x{multiplier:.2f} = ${current_win:.2f})", callback_data=f"crash_cashout_{bet}_{multiplier}_{msg.message_id}"))
        try:
            await msg.edit_text(f"🚀 *КРАШ*\n━━━━━━━━━━━━━━━━━━━━\nСтавка: `${bet:.2f}`\n\n📈 Ракета летит!\n\nМножитель: *x{multiplier:.2f}*\nПотенциальный выигрыш: `${current_win:.2f}`", parse_mode="Markdown", reply_markup=builder.as_markup())
        except Exception:
            break
    try:
        await db.record_game(user_id, "crash", bet, 0, False)
        new_balance = await db.get_balance(user_id)
        await msg.edit_text(f"💥 *КРАШ на x{crash_point:.2f}!*\n━━━━━━━━━━━━━━━━━━━━\nРакета упала!\n\nСтавка: `${bet:.2f}`\nПроигрыш: `${bet:.2f}`\n💰 Баланс: `${new_balance:.2f}`", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔄 Играть снова", callback_data="game_crash"), InlineKeyboardButton(text="🔙 Меню", callback_data="games_menu")]]))
    except Exception:
        pass


async def run_blackjack_from_message(message: types.Message, bet: float):
    ...
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="➕ Взять карту", callback_data=f"bj_hit_{bet}"), InlineKeyboardButton(text="✋ Стоп", callback_data=f"bj_stand_{bet}"))
    builder.row(InlineKeyboardButton(text="⬆️ Удвоить", callback_data=f"bj_double_{bet}"))
    await message.answer(text, parse_mode="Markdown", reply_markup=builder.as_markup())


async def run_bulls_from_message(message: types.Message, bet: float):
    user_id = message.from_user.id
    balance = await db.get_balance(user_id)
    if balance < bet:
        await message.answer(f"❌ Недостаточно средств!")
        return
    text = f"🎯 *БЫКИ И МЕДВЕДИ*\n━━━━━━━━━━━━━━━━━━━━\nСтавка: `${bet:.2f}`\n\nУгадай движение BTC за следующие 30 секунд!\n\n📈 Бык — цена вырастет\n📉 Медведь — цена упадёт\n\nВыигрыш: x1.9\n━━━━━━━━━━━━━━━━━━━━"
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="📈 Бык", callback_data=f"bulls_up_{bet}"), InlineKeyboardButton(text="📉 Медведь", callback_data=f"bulls_down_{bet}"))
    await message.answer(text, parse_mode="Markdown", reply_markup=builder.as_markup())
...
"""
    ...
    await edit_message(call, text, builder.as_markup())
    await call.answer()
...
@dp.message(States.waiting_deposit_amount)
async def process_custom_deposit(message: types.Message, state: FSMContext):
    try:
        amount = float(message.text.replace("$", "").strip())
        if amount < 5:
            await message.answer("❌ Минимальная сумма: $5")
            return
        if amount > 10000:
...
@dp.callback_query(F.data.startswith("with_all_"))
async def withdraw_all(call: types.CallbackQuery, state: FSMContext):
    amount = float(call.data.split("_")[2])
    if amount < 10:
        await call.answer("❌ Минимальная сумма вывода: $10", show_alert=True)
        return
...
"""
    ...
    builder.row(InlineKeyboardButton(
        ...
        url=f"https://t.me/share/url?url={ref_link}&text=Играй%20в%20HiBet%20Casino%20и%20выигрывай!"
    ))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
...
async def history_handler(call: types.CallbackQuery):
    ...
    if not games_history:
        ...
        for g in games_history:
            ...
            win_str = f"${g['win']:.0f}" if g['won'] else f"-${g['bet']:.0f}"
            rows.append(f"{emoji} {game_emoji} ${g['bet']:.0f} → {win_str}")
...
"""
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="💬 Написать в поддержку", url="https://t.me/hibetsup"))
    builder.row(InlineKeyboardButton(text="📣 Переходник", url="https://t.me/hiibet"))
    builder.row(InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu"))
...
@dp.message(AdminStates.add_balance_id)
async def admin_add_balance_id(message: types.Message, state: FSMContext):
    try:
        user_id = int(message.text.strip())
        await state.update_data(target_id=user_id)
        await state.set_state(AdminStates.add_balance_amount)
        await message.answer(f"✅ ID: `{user_id}`\n\nВведите сумму для начисления (USD):", parse_mode="Markdown")
    except ValueError:
...
async def admin_add_balance_amount(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"✅ *Готово!*\n\nПользователю `{target_id}` начислено `${amount:.2f}`\nНовый баланс: `${new_bal:.2f}`", parse_mode="Markdown")
        try:
            await bot.send_message(target_id, f"💰 *Администратор начислил вам ${amount:.2f}!*\n\nВаш баланс: `${new_bal:.2f}`", parse_mode="Markdown")
        except Exception:
            pass
    except ValueError:
...
@dp.message(AdminStates.ban_id)
async def admin_do_ban(message: types.Message, state: FSMContext):
    try:
        user_id = int(message.text.strip())
        await state.clear()
        async with __import__('aiosqlite').connect('casino.db') as db_conn:
            await db_conn.execute("UPDATE users SET is_banned = 1 WHERE user_id = ?", (user_id,))
...
async def admin_promo_start(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id not in ADMIN_IDS:
        return
    await state.set_state(AdminStates.promo_code)
    await call.message.answer(
        "🎫 *Создание промокода*\n\nВведите код (например: SUMMER2025):",
        parse_mode="Markdown"
    )
    await call.answer()
...
async def admin_promo_code(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ Код: `{code}`\n\nВведите сумму бонуса (USD):", parse_mode="Markdown")
...
async def admin_promo_amount(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(f"✅ Сумма: `${amount}`\n\nВведите кол-во использований:", parse_mode="Markdown")
    except ValueError:
        await message.answer("❌ Введите корректную сумму")
    ...
    try:
        ...
        async with __import__('aiosqlite').connect('casino.db') as db_conn:
            await db_conn.execute(
                "INSERT OR REPLACE INTO promo_codes (code, amount, uses_left, used_by) VALUES (?, ?, ?, '[]')",
                (code, amount, uses)
            )
            await db_conn.commit()
        await message.answer(
...
async def admin_stats(call: types.CallbackQuery):
    ...
    users = await db.get_all_users()
    text = f"""
📊 *ПОЛНАЯ СТАТИСТИКА — HIBET*
━━━━━━━━━━━━━━━━━━━━
👥 Всего игроков: `{stats['total_users']}`
🚫 Активных: `{len(users)}`
🎮 Игр сыграно: `{stats['total_games']}`
━━━━━━━━━━━━━━━━━━━━
💰 Общий оборот: `${stats['total_turnover']:.2f}`
💸 Выплачено игрокам: `${stats['total_paid']:.2f}`
📈 Прибыль казино: `${stats['casino_profit']:.2f}`
━━━━━━━━━━━━━━━━━━━━
"""
